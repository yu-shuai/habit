...
import { Check, Users, User, X, Flame, AlertTriangle, Lock } from 'lucide-react';
...
export default function HabitCard({ habit, onCheck, onDelete }: HabitCardProps) {
  ...
  const cardClass = isFailed
    ...
    : isCompleted
      ...
      : isPenalty
        ...
        : 'bg-neutral-900 border-neutral-800';   // 全部统一深色
  ...
  const canCheck = !isFailed && (!habit.isCompletedToday || isCompleted) && !(isTeam && !habit.isStarted);
  ...
  return (
    <motion.div
      ...
      whileHover={!isFailed ? { y: -3, boxShadow: '0 12px 32px rgba(0,0,0,0.25)' } : {}}
      transition={{ type: 'spring', stiffness: 280, damping: 24 }}
      onContextMenu={e => { e.preventDefault(); setShowDelete(v => !v); }}
      className={`relative rounded-2xl px-5 py-4 border flex items-center gap-4 text-white select-none overflow-hidden ${isCompleted ? 'cursor-pointer' : ''} ${cardClass}`}
      onClick={() => isCompleted && canCheck && onCheck(habit.id, true)}
    >
      ...
      <div className="relative flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mb-1 flex-wrap">
          ...
          {isTeam && !habit.isStarted && !isFailed && (
        ...
        {/* Progress text */}
        <p className="text-[11px] font-bold text-white/40 mt-1.5">
      ...
      <div className="relative flex items-center gap-2 flex-shrink-0">
        ...
        {isPenalty && !isFailed ? (
          ...
          <motion.button
            ...
            aria-label={habit.isCompletedToday ? '已打卡' : '打卡'}
          >
            <Check
              size={24}
              strokeWidth={3}
              className={habit.isCompletedToday || isCompleted ? 'text-white' : 'text-white/50'}
            />
          </motion.button>
        )}
      </div>

      {/* Top Right: Delete/Lock button */}
      <AnimatePresence>
        {showDelete && (
          <motion.button
            ...
            className={`absolute top-2.5 right-2.5 w-6 h-6 flex items-center justify-center z-20 transition-opacity hover:opacity-70 active:scale-90 ${
              isTeam && habit.isStarted && !isFailed && !habit.isArchived
                ? 'opacity-20'
                : ''
            }`}
          >
